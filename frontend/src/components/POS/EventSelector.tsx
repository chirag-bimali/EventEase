...
export const EventSelector = ({ onSelect }: EventSelectorProps) => {
  const [filter] = useState<GetAllEventsFilters>({
    statusNot: EventStatus.DRAFT,
  });
  const { events, loading, error } = useEvents(filter); // Only fetch published events for selection
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {filteredEvents.map((event) => (
          ...
          >
            ...
            {/* Select Button */}
            <button className="mt-4 w-full bg-purple-600 text-white py-2 px-4 rounded-md hover:bg-purple-700 transition-colors font-medium cursor-pointer">
              Select Event
            </button>
          </div>
        ))}
